...
import { addToStorage, getStoredList, removeFromStorage } from "../../utilities/fakeDb";
...
const MainContent = () => {
  ...
  const addToListBtn = (selectedCountry) => {
    let newCountyList = []
    const existsCountry = countryList.find(country => country.cca3 === selectedCountry.cca3)
    if(!existsCountry) {
      selectedCountry.quantity = 1;
      newCountyList = [...countryList, selectedCountry]
    }
    else {
        const rest = countryList.filter(country => country.cca3 !== selectedCountry.cca3)
  ...
  return (
    <div className="main-content">
      ...
      <div>
        ...
        <CountryList countryList={countryList} removeStorage={removeStorage}></CountryList>
